...
Policy Compliance Analyst is a complete full-stack AI project for **Agentic RAG** at **Intermediate** difficulty. It includes a FastAPI backend, a React frontend, sample documents, vector search with ChromaDB support, source citations, timeline logs, structured run storage, Docker files, and tests.

{'description': 'The Policy Compliance Analyst is designed to assist organizations in identifying and mitigating regulatory risks by analyzing policies, procedures, and regulatory requirements.', 'keywords': ['Policy Compliance', 'Regulatory Risk', 'Risk Management']}
...
- Selected architecture: Policy Compliance Analyst
...
This generated project satisfies **Agentic RAG** through the runtime path below. The implementation is not only a README claim: the files listed after the diagram are generated in the repository and validated before GitHub push.
...
- `backend/app/services/pipeline.py` orchestrates the project-type flow: Planner Agent, Retriever Agent, Reasoning Agent, Reviewer Agent.
...
- Flow style: Planner -> retriever -> reasoning step -> optional tool call -> final answer with sources and timeline.
...
```text
ai-agentic-rag-policy-compliance-analyst/
  backend/
app/
  main.py
  config.py
...
- Upload Policy: User uploads a policy and procedure
- Assess Regulatory Risks: Risk Assessment Engine assesses regulatory risks
- Display Results: Risk Assessment Results feature displays risk assessment results

## Business Rules

- Risk Assessment Engine assesses regulatory risks when a policy is uploaded
- Risk Assessment Results feature displays risk assessment results when regulatory risks are identified
